...
fun FaRapidItemDetail.toDomain(
    ...
    originalTitle = if (title != null) title.original?.takeIf { it != title.local } else null,
    ...
    writers = credits?.writers.orEmpty(),
    cast = credits?.cast?.mapNotNull { it.name }.orEmpty(),
    music = credits?.music.orEmpty(),
    cinematography = credits?.photography.orEmpty(),
    productionCompanies = credits?.producers.orEmpty(),
